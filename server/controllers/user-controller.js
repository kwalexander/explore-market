...
module.exports = {
  ...
  // save a travel to a user's `savedtravels` field by adding it to the set (to prevent duplicates)
  // user comes from `req.user` created in the auth middleware function
  async saveTravel({ user, body }, res) {
    console.log(user);
    try {
      const updatedUser = await User.findOneAndUpdate(
        { _id: user._id },
        { $addToSet: { savedtravels: body } },
        { new: true, runValidators: true }
      );
      return res.json(updatedUser);
    } catch (err) {
  ...
  // remove a travel from `savedtravels`
  async deleteTravel({ user, params }, res) {
    const updatedUser = await User.findOneAndUpdate(
      { _id: user._id },
      { $pull: { savedtravels: { travelID: params.travelID } } },
      { new: true }
    );
    if (!updatedUser) {
      return res.status(404).json({ message: "Couldn't find user with this id!" });
